...
class CommentService {
    async deleteComment(id: string) {
        return await axios.delete(`/Comments/${id}`, { headers: authHeader() });
    }

    async createComment(data: FormData): Promise<Comm> {
        const axiosResponse: AxiosResponse = await axios.post(`/Comments`, data, { headers: { ...authHeader(), 'content-type': 'application/json' } });
    ...
    async loadComments(postId: string, pageNumber: number): Promise<Comments> {
        const axiosResponse: AxiosResponse = await axios.get(`/Comments`, {params: {postId: postId, pageNumber: pageNumber}, headers: authHeader() });
